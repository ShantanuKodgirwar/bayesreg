...
class Test(unittest.TestCase):
    ...
    def test_OLS_fit(self):
        ...
        np.testing.assert_almost_equal(
            calc_lsq_estimator(x_train, y_train, n_degree),
...
if __name__ == "__main__":
    ...
    train_error_OLS, test_error_OLS = calc_rmse_lsq(
        x_train, y_train, x_test, y_test, n_degree
    )
    ...
    ax.plot(
        x_train, calc_lsq_estimator(x_train, y_train, n_degree), label="training fit"
    )
    ax.plot(
        x_test,
        calc_lsq_estimator(x_test, y_test, n_degree),
        color="r",
        label="testing fit",
    )
    ax.set_xlabel(r"$x_n$")
    ...
    ax.plot(
        np.arange(1, len(x_train)), test_error_OLS, label="testing error", color="r"
    )
