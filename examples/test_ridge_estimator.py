...
class Test(unittest.TestCase):
    ...
    def test_ridge_fit(self):
        ...
        np.testing.assert_almost_equal(
            calc_ridge_estimator(x_train, y_train, n_degree, ridge_param),
            calc_ridge_estimator_sklearn(x_train, y_train, n_degree, ridge_param),
            decimal=3,
        )
...
if __name__ == "__main__":
    ...
    train_error_ridge, test_error_ridge = calc_rmse_ridge(
        x_train, y_train, x_test, y_test, n_degree, lambda_logspace
    )
    ...
    print(
        f"Best fit ridge parameter (log scale) is {lambda_vals[np.argmin(test_error_ridge)]}"
    )
